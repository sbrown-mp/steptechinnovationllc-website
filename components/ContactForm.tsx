...
type ContactFormValues = LeadFormValues & {
...
export function ContactForm({ inquiryType, sourcePage, submitLabel, successMessage }: ContactFormProps) {
  ...
  const onSubmit = handleSubmit(async (values) => {
    ...
    try {
      const payload =
        inquiryType === "audit"
          ? {
              ...
              primaryImprovement: values.primaryImprovement,
              biggestBottleneck: values.biggestBottleneck,
              currentTools: values.currentTools?.trim() ? values.currentTools.trim() : "",
            }
          : {
              ...values,
      ...
      const response = await fetch(inquiryType === "audit" ? "/api/audit-request" : "/api/lead", {
        method: "POST",
        headers: {
  ...
  return (
    <form onSubmit={onSubmit} className="glass-card space-y-5 p-6 md:p-8" noValidate>
      ...
      {inquiryType === "audit" ? (
        <label className="block text-sm font-medium text-slate-200">
          What are you primarily looking to improve?
          <select
            {...register("primaryImprovement", {
              required: "Please select what you are primarily looking to improve.",
            })}
            className={cn(fieldClass, "pr-10")}
          >
            <option value="">Select one</option>
            {primaryImproveOptions.map((option) => (
              <option key={option} value={option}>
                {option}
              </option>
            ))}
          </select>
          {errors.primaryImprovement ? (
            <span className="mt-1 block text-xs text-rose-300">{errors.primaryImprovement.message}</span>
          ) : null}
        </label>
      ) : null}
      ...
      <p className="text-xs text-slate-400" aria-live="polite">
        {inquiryType === "audit" ? "No obligation. We'll respond within one business day." : "We typically respond within one business day."}
      </p>
    </form>
  );
}
